...
public class ProcessoResource implements Serializable {
	...
	@GetMapping(value = "/{isnProcesso}")
	private ResponseEntity<?> findOne(@PathVariable("isnProcesso") Long isnProcesso) {
		try {
			return ResponseEntity.ok(processoService.findOne(isnProcesso).orElse(null));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}
